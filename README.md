...
ELB log analyzer is a command line tool for downloading and parsing Elastic Load Balancer's access logs and getting quick statistics. Useful for detecting requests taking longest time, IPs making most requests and many other data that can be derived from log files. 

The script will fetch available clients from DynamoDb table and is using cross account role for accessing an AWS account. 
## Installation
```sh
```
...
python elb_log_analysis.py

By default log analyzer will group by Request_URL and sort by latency. This functionality can be changed  but this one was chosen as default behaviour since it appears to be the most common case. Example output:


Request_Url | Request_Processing_Time | Backend_Processing_Time | Response_Processing_Time | ELB_Status_Code | Count
------------ | ------------- | ---------------------------- | ------------------- | ---------------- | -------------- 
GET https://preview.spendhq.com:443/login HTTP/1.1 | 3.6e-05 | 0.388242 | 5.2e-05 | 200 | 122
HEAD https://preview.spendhq.com:443/ HTTP/1.1 | 4.5e-05 | 0.314439 | 3.9e-05 | 302 | 1416
...
#### Version
`--version` option returns the version 
```sh
elb_log_analysis --version
```
Example Output:
```
elb-log-analysis 1.0.0
```
